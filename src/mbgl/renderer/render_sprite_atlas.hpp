...
typedef std::unordered_map<std::string, SpriteAtlasElement> IconMap;
...
class RenderSpriteAtlas : public util::noncopyable {
public:
    using Images = std::unordered_map<std::string, std::shared_ptr<const style::Image>>;

    RenderSpriteAtlas(Size, float pixelRatio);
    ~RenderSpriteAtlas();

    void onSpriteLoaded(const Images&);
    ...
    void addImage(const std::string&, std::shared_ptr<const style::Image>);
...
private:
    ...
    struct Entry {
        std::shared_ptr<const style::Image> image;
