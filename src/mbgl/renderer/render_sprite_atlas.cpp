...
RenderSpriteAtlas::RenderSpriteAtlas(Size size_, float pixelRatio_)
    : size(std::move(size_)),
...
RenderSpriteAtlas::~RenderSpriteAtlas() = default;

void RenderSpriteAtlas::onSpriteLoaded(const Images& result) {
    loaded = true;
...
void RenderSpriteAtlas::addImage(const std::string& id, std::shared_ptr<const style::Image> image_) {
    icons.clear();
...
void RenderSpriteAtlas::removeImage(const std::string& id) {
    icons.clear();
...
const style::Image* RenderSpriteAtlas::getImage(const std::string& id) const {
...
void RenderSpriteAtlas::getIcons(IconRequestor& requestor) {
    if (isLoaded()) {
        requestor.onIconsAvailable(buildIconMap());
    } else {
...
void RenderSpriteAtlas::removeRequestor(IconRequestor& requestor) {
    requestors.erase(&requestor);
}

optional<SpriteAtlasElement> RenderSpriteAtlas::getIcon(const std::string& id) {
    return getImage(id, &Entry::iconRect);
}

optional<SpriteAtlasElement> RenderSpriteAtlas::getPattern(const std::string& id) {
    return getImage(id, &Entry::patternRect);
}

optional<SpriteAtlasElement> RenderSpriteAtlas::getImage(const std::string& id,
...
void RenderSpriteAtlas::copy(const Entry& entry, optional<Rect<uint16_t>> Entry::*entryRect) {
    if (!image.valid()) {
...
IconMap RenderSpriteAtlas::buildIconMap() {
    if (icons.empty()) {
...
void RenderSpriteAtlas::upload(gl::Context& context, gl::TextureUnit unit) {
    if (!texture) {
...
void RenderSpriteAtlas::bind(bool linear, gl::Context& context, gl::TextureUnit unit) {
    upload(context, unit);
